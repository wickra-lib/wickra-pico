...
checks its packages here too (they are staged at publish time); wickra-pico is
firmware whose release attaches a `.uf2` and publishes no package, so the check
covers the two host crates that could go to crates.io and stops there.
...
# The C ABI crate is not published to crates.io (publish = false); its
# staticlib ships as a release archive that packages the licence texts from
# this directory, so they have to be here.
EXTRA = ()
