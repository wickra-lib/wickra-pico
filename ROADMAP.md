...
- **Signal kernel + host reference** — the EMA(9)/EMA(21) cross over `embed-core`,
...
- **More indicators** — expose the rest of `embed-core` (RSI, ATR) on-device.
- **`wickra-embed` facade** — extract a dedicated, documented `no_std` facade
  crate over `embed-core` for downstream reuse.
